...
public class SpaceExplorer extends Thread {
	...
	public void run() {
		while(true) {
			Message mes = channel.getMessageHeadQuarterChannel();
			if (!discovered.contains(mes.getCurrentSolarSystem())) {
				if(mes.getData().equals(HeadQuarter.EXIT)){
					break;
				}
				Message messRet = new Message(mes.getParentSolarSystem(), mes.getCurrentSolarSystem(), encryptMultipleTimes(mes.getData(), hashCount));
				channel.putMessageSpaceExplorerChannel(messRet);
			}
			discovered.add(mes.getCurrentSolarSystem());
		}
	}
