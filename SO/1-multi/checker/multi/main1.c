...
disp_map (struct mp *map)
{
    ...
    {
      fprintf (log_file, "%s : %s ", map->lst[i].key, map->lst[i].val);
	      ...
	      char *poz) 
{
  ...
  char *multiple = strchr (input->mtrx[*i], '\\');
  for (int cnt = 0; multiple != NULL || last; cnt++)
    {
      input->prnt[*i] = 0;
      char *curr_line = (cnt == 0 ? poz : input->mtrx[*i]);
      int pass = 0;
      for (int j = 0; j < strlen (curr_line); j++)
	{
...
replace_define (struct mp *map, struct matrx *input) 
{
  ...
  int found_main = 1, pass = 1;
  for (int i = 0; i < input->cnt; i++)
    {
      poz = strstr (input->mtrx[i], "#ifdef ");
      if (poz != NULL)
	solve_ifdef (map, input, &found_main, &i, poz);
      ...
      while (pass)
	{
	  ...
	  for (int j = 0; j < map->cnt; j++)
	    {
	      poz = strstr (input->mtrx[i], map->lst[j].key);
	      if (poz != NULL && map->lst[j].type == TYPE_D
		   && check_var (input->mtrx[i], poz) && input->prnt[i])
		{
		  ...
		  strncat (buff, poz + strlen (map->lst[j].key),
	...
	solve_if (map, input, &found_main, &i, poz);
...
main (int argc, char *argv[]) 
{
  ...
  print_out (ret, &input, out_file_path);
