...
Obstacle::Obstacle(){
	obst = Object2D::CreateRect("middle", glm::vec3(0, 0, 0), lengthObstacle, widthObstacle, glm::vec3(0, 1, 0), true);
}
...
vector<pair<float, float>> Obstacle::getOrigin() {
	return origin;
}
...
vector<pair<Mesh *, glm::mat3>> Obstacle::getMeshes() {
	...
	for (auto &it : origin) {
		...
		ret.push_back(make_pair(obst, modelMatrix));
	}
